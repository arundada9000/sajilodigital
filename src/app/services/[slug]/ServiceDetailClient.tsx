...
export default function ServiceDetailClient({
  ...
  return (
    <div
      ref={containerRef}
      className="bg-black text-white min-h-screen selection:bg-blue-500 selection:text-white overflow-x-hidden"
    >
      {/* Navigation */}
      <nav className="fixed top-0 left-0 w-full z-50 p-6 md:p-8 flex justify-between items-center mix-blend-difference">
        ...
        <div className="hidden md:flex gap-12">
          {["Strategy", "Performance", "Creative", "Scale"].map((item) => (
            <span
              key={item}
              className="text-[9px] tracking-[0.5em] font-black uppercase text-white/20"
            >
              {item}
            </span>
          ))}
      ...
      <section className="relative h-[80vh] md:h-[90vh] flex flex-col items-center justify-center px-6 overflow-hidden">
        ...
        >
          ...
          <span className="text-[8px] md:text-[10px] tracking-[0.5em] font-black uppercase text-white/20">
            Data Stream
          </span>
        </motion.div>
      </section>
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8">
              {[
                ...
                <div
                  key={m.label}
                  className="border-l border-white/10 pl-5 md:pl-6 py-2"
                >
                  <span className="text-[8px] tracking-widest uppercase font-bold text-white/20 block mb-1">
                    {m.label}
                  </span>
      ...
      <section className="py-24 md:py-40 bg-white/[0.02] border-y border-white/5">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16 md:mb-24">
            <span className="text-[10px] md:text-[11px] tracking-[0.5em] md:tracking-[0.6em] font-black uppercase text-white/20 block mb-6">
              Investment Models
            </span>
            <h2 className="text-4xl md:text-8xl font-black italic uppercase tracking-tighter">
              Strategic <span className="text-white/20">Packages.</span>
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            {service.pricing.map((plan: PricingPlan, i: number) => (
              ...
              >
                ...
                <h4 className="text-[10px] md:text-[11px] tracking-[0.3em] md:tracking-[0.4em] font-black uppercase text-white/40 mb-8 md:mb-10">
                  {plan.plan}
                </h4>
                <div className="mb-8 md:mb-10">
                  ...
                  <span className="block text-[9px] md:text-[10px] uppercase font-bold text-white/20 mt-2 tracking-widest">
                    {plan.duration}
                  </span>
                </div>
                ...
                <Link href="/contact" className="block">
                  <button
                    className={`w-full py-4 md:py-5 rounded-xl md:rounded-2xl text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] md:tracking-[0.4em] transition-all flex items-center justify-center gap-3 md:gap-4 ${plan.popular ? "bg-white text-black hover:scale-105 shadow-2xl shadow-blue-500/20" : "bg-white/5 text-white hover:bg-white hover:text-black"}`}
                  >
      ...
      <footer className="p-8 md:p-12 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-8">
        <span className="text-[9px] md:text-[10px] tracking-[0.5em] md:tracking-[1em] font-black text-white/10 uppercase">
          Sajilo.Digital 2026
        </span>
        <div className="flex gap-6 md:gap-10">
          {["Instagram", "Facebook", "Github"].map((s) => (
            <a
              key={s}
              href="#"
              className="text-[9px] md:text-[10px] tracking-widest font-bold text-white/30 hover:text-white transition-colors uppercase"
            >
              {s}
            </a>
          ))}
