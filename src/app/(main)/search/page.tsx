      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-12">
          <motion.h1
            ...
            className="text-4xl md:text-6xl font-black italic uppercase tracking-tighter text-foreground mb-4"
          >
          ...
          <motion.p
            ...
            className="text-foreground/50 font-mono text-sm uppercase tracking-widest"
          >
            {isSearching
              ? "Decrypting database..."
          ...
          {results.length > 0 ? (
            results.map((result, index) => (
              ...
              >
                <Link
                  href={result.href}
                  className="group flex items-center justify-between p-6 bg-foreground/5 border border-border rounded-xl hover:bg-foreground/10 hover:border-blue-500/50 transition-all duration-300 backdrop-blur-md"
                >
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <span className="text-[10px] font-mono uppercase tracking-wider px-2 py-0.5 bg-foreground/10 rounded text-foreground/40">
                          {result.type}
                        </span>
                  ...
                  <ArrowRight className="w-5 h-5 text-foreground/20 group-hover:text-blue-500 group-hover:translate-x-1 transition-all" />
                </Link>
              </motion.div>
            ))
          ) : (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="py-20 text-center border border-dashed border-border rounded-xl bg-foreground/5 backdrop-blur-md"
            >
...
export default function SearchPage() {
  return (
    <div className="min-h-screen bg-surface-deep text-foreground selection:bg-blue-500/30">
      <Grain />
      <Suspense
        fallback={
