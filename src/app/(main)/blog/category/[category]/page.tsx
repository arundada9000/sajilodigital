...
export default async function CategoryPage({
  ...
  return (
    <div className="min-h-screen bg-[#0b0f19] text-white selection:bg-cyan-500/30 overflow-x-hidden pt-28">
      ...
      <section className="relative px-6 pb-8">
        <div className="max-w-7xl mx-auto">
          <nav className="flex items-center gap-3 text-sm font-medium text-gray-500 uppercase tracking-widest mb-12">
              ...
              <Link key={post.slug} href={`/blog/${post.slug}`} className="group relative">
                <div className="relative h-full flex flex-col rounded-3xl border border-white/10 bg-[#161b22]/40 backdrop-blur-xl overflow-hidden hover:border-cyan-500/50 transition-all duration-300 shadow-xl">
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    <h2 className="text-xl font-bold text-white mb-3 group-hover:text-cyan-400 transition-colors duration-300 line-clamp-2">{post.title}</h2>
                    <p className="text-gray-400 text-sm line-clamp-2 leading-relaxed flex-1">{post.excerpt}</p>
