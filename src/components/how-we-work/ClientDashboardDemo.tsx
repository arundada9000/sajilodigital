    ...
    return (
        ...
        >
            <motion.div
                ...
                className="rounded-xl overflow-hidden border border-border bg-surface-alt shadow-2xl relative group transition-shadow duration-500 hover:shadow-cyan-500/20"
            >
                {/* Glare Effect */}
                <motion.div
                    style={{
                ...
                {/* Top Bar */}
                <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-background/40 backdrop-blur-sm relative z-40">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs font-mono text-muted-foreground">client-portal.exe</span>
                    </div>
                ...
                <div className="flex min-h-[600px] relative z-30 bg-surface-alt">
                    {/* Sidebar */}
                    <div className="w-64 border-r border-border bg-background/20 p-4 hidden md:block">
                        <div className="mb-8">
                            <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-4">Project</h3>
                            <div className="p-3 rounded-lg bg-foreground/5 border border-border/5 group-hover:border-border/10 transition-colors">
                                <div className="font-bold text-foreground mb-1">E-Commerce Redesign</div>
                        ...
                        <div className="mt-8 p-4 rounded-lg bg-purple-500/10 border border-purple-500/20">
                            ...
                            <div className="h-1.5 w-full bg-foreground/10 rounded-full overflow-hidden">
                                <div className="h-full bg-purple-500 rounded-full w-[42%] relative overflow-hidden">
                                    <div className="absolute inset-0 bg-foreground/20 animate-[shimmer_2s_infinite] skew-x-12" />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {/* Main Content */}
                    <div className="flex-1 p-6 md:p-8 bg-surface relative overflow-hidden flex flex-col h-[600px]">
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                {STAGES.map((stage) => (
                    <div key={stage.id} className="p-4 rounded-lg border border-border/5 bg-foreground/[0.02] hover:bg-foreground/[0.04] transition-colors">
                        ...
                        <div className="font-medium text-foreground mb-2">{stage.name}</div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 flex-1 min-h-0">
                ...
                <div className="lg:col-span-2 flex flex-col h-full">
                    ...
                    <div className="rounded-lg border border-border bg-background/50 p-4 font-mono text-sm flex-1 overflow-y-auto custom-scrollbar">
                        {RECENT_LOGS.map((log, i) => (
                            <div key={i} className="flex gap-4 py-2 border-b border-border/5 last:border-0 hover:bg-foreground/5 px-2 rounded transition-colors">
                                ...
                                <span className={cn(
                                    ...
                                    log.type === "code" ? "text-blue-300" :
                                        log.type === "asset" ? "text-purple-300" :
                                            log.type === "design" ? "text-pink-300" : "text-foreground/70"
                                )}>
...
function CodeTab() {
    return (
        <div className="h-full flex flex-col">
            ...
            <div className="flex-1 rounded-lg border border-border bg-surface p-4 font-mono text-sm overflow-hidden relative">
                <div className="flex items-center gap-2 mb-4 border-b border-border/10 pb-2">
                    <div className="w-2 h-2 rounded-full bg-red-500" />
                    <div className="w-2 h-2 rounded-full bg-yellow-500" />
                    <div className="w-2 h-2 rounded-full bg-green-500" />
                    <span className="ml-2 text-muted-foreground text-xs">src/components/Hero.tsx</span>
                </div>
                <div className="text-blue-400">import</div> <span className="text-foreground">React</span> <div className="text-blue-400 inline">from</div> <span className="text-orange-300">"react"</span>;
                <br />
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            <div className="grid grid-cols-2 gap-4">
                {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="aspect-video rounded-lg bg-foreground/5 border border-border/10 flex items-center justify-center relative group overflow-hidden cursor-pointer">
                        ...
                        <ImageIcon className="w-8 h-8 text-muted-foreground group-hover:text-white transition-colors" />
                        <span className="absolute bottom-2 left-2 text-xs text-muted-foreground group-hover:text-white">Asset_0{i}.png</span>
                    </div>
                ))}
            </div>
        </div>
...
function DocsTab() {
    return (
        <div className="h-full flex flex-col">
            ...
            <div className="flex-1 rounded-lg border border-border bg-foreground/5 p-8 max-w-3xl mx-auto w-full">
                <h1 className="text-2xl font-bold mb-4 border-b border-border/10 pb-4">Project Requirements</h1>
...
function SidebarItem({ icon: Icon, label, active, onClick }: any) {
    return (
        <button
            ...
            className={cn(
                "w-full flex items-center gap-3 px-3 py-2 rounded-md transition-all text-sm font-medium focus:outline-none focus:ring-2 focus:ring-border/10",
        ...
        <div style={{ marginLeft: level * 12 }}>
            {items.map((item, i) => (
                <div key={i} className="mb-1">
                    <div className="flex items-center gap-2 text-muted-foreground hover:text-foreground cursor-default">
                        {item.type === "folder" ? (
                            <>
