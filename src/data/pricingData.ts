...
export const servicePricing: ServicePricing[] = services.map((service) => ({
  ...
  accentColor: service.accentColor ?? "text-gray-500",
  category: service.pricing[0].duration.toLowerCase().includes("month")
    ? "monthly"
    : "one-time",
...
function generateComparisonData(
  ...
  const commonComparisons: Record<string, ComparisonRow[]> = {
    ...
    "app-development": [
      {
        feature: "Platform",
        tier1: "Cross-platform",
        tier2: "Cross-platform",
        tier3: "Native",
      },
      {
        feature: "Features",
...
export const packageBundles: PackageBundle[] = [
  ...
  },
  {
    ...
    name: "Growth Bundle",
    description:
      "Accelerate your business with comprehensive digital solutions",
...
export function calculateBundleSavings(bundleId: string): {
  saved: string;
  percentage: number;
} {
